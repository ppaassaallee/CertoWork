...
Personal notes in `knowledge_items` stay owner-scoped. Project Docs (`projectId` set) are readable by joined workspace members. The client merges the owner query with `workspaceId` + `projectId != ""`.
