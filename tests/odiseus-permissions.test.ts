...
test("firestore rules allow Odiseus run and activity collections", () => {
...
test("chat persistence does not hard-fail on Odiseus run writes", () => {
  const source = readFileSync(
    resolve("src/components/DelivereeWorkspace.tsx"),
    "utf8",
  );
  assert.match(source, /persistOdiseusRun\(/);
  assert.match(source, /recordOdiseusActivitySafe\(/);
  assert.doesNotMatch(
    source,
